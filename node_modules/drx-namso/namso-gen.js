module.exports.gen = (obj) => {
	...
	if (bin !== "" && bin != null) {
	  ...
	  for (let k = 1; k <= Quantity; k++) {
		...
		const cn = chkCard(p);
		let clcd, ccck; // Declarar las variables aquí
		let cdi; // Mover la declaración de la variable aquí
		for (let i = tr; i >= 1; i--) {
		  output2 = "Generating...";
		...
		if (clcd && ccck) {
		  const cn = chkCard(cdi);
		  let ccv;
		  if (showCCV) {
			if (obj.CCV === 'rnd') {
				const binC = cdi.toString();
				const tipo = parseInt(binC.substring(0, 1));
				if (tipo === 3) ccv = String(Math.floor(Math.random() * (9999 - 0 + 1))).padStart(4, '0');
				else ccv = String(Math.floor(Math.random() * 1000)).padStart(3, '0');
			  } else {
				ccv = obj.CCV;
			  }
		  } else {
		  ...
		  let mes, year;
			if (showExpDate) {
			if (Month === 'rnd' && Year === 'rnd') {
				const dnowi = new Date();
				const currentYear = dnowi.getFullYear();
				const maxValidity = 5; // Maximum validity of the card in years
				mes = unilenS(rnd(1, 12), 2, '0', 0);
				year = currentYear + rnd(1, maxValidity);
			} else if (Month !== 'rnd' && Year === 'rnd') {
				const dnowi = new Date();
				mes = Month;
				year = (dnowi.getFullYear() + rnd(2, 5));
			} else if (Month === 'rnd' && Year !== 'rnd') {
				const dnowi = new Date();
				mes = unilenS(rnd(1, 12), 2, '0', 0);
				year = Year;
			...
			// Convertir los últimos 2 dígitos del año en 4 dígitos si es necesario
			if (year.length === 2) {
		  ...
		  let outLine = cdif;
		  if (showExpDate) outLine += "|" + mes + "|" + year;
		  if (showCCV) outLine += "|" + ccv;
		  if (showBank) outLine += "|" + cn;
		  out += outLine + "\n";
		} else {
		  out = "Sorry, no valid BIN entered, or BIN is incomplete, please check the format is correct, example: '666666xxxxxxxxxxxx'";
		  break;
		}
	  }
	  if (jou === 2) out += "</xml>";
